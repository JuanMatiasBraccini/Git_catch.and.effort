...
#       Analyses inlcude the different managament zones in one single model despitefew vessels (12%)
#          fishing in > 1 zone (between 2006-2012) and fishers have different targeting practices 
#       and species have different stock structures in the different zones(same as Punt et al 2000)

#       Dusky includes C. obscurus and C. brachyurus (mention in paper)

# Error structures: Delta-lognormaland Delta-gamma as response variable is catch rate.
...
#MISSING:     
#             export index for zones combined, see "#for each species and combined zones"
#             export SE in logspace (this is the CV used by SS3)
...
#Mean Freo sea level
Freo=read.csv("C:/Matias/Data/Freo_mean_sea_level.csv")  #note: from 10/2013 is dummy because the data were not
                                              #available, however, Freo sea level is not a significant term
names(Freo)[3]="Freo"


#Fishable areas
Whis.fishArea=read.csv("C:/Matias/Data/Catch and Effort/FishableArea/BLOCKX_whiskery.csv")
Gum.fishArea=read.csv("C:/Matias/Data/Catch and Effort/FishableArea/BLOCKX_gummy.csv")
Dusky.fishArea=read.csv("C:/Matias/Data/Catch and Effort/FishableArea/BLOCKX_dusky.csv")
Sand.fishArea=read.csv("C:/Matias/Data/Catch and Effort/FishableArea/BLOCKX_sandbar.csv")
...
Fish.Pow=.02  #annual effort creep
...
#Control type of business rule for filling missing last year of data
...
if(Model.run=="Standard") Do.boot="YES"
if(Model.run=="First") Do.boot="NO"

N.boot=1000
#N.boot=10


#Control if separating monthly from daily records
Separate.monthly.daily="NO"
...
fn.plot.folly=function(a,spec,CPUE.threshold,CPUE.threshold1) #function for plotting folly example
{
  ...
  a$cpue=a$LIVEWT.c/a$Km.Gillnet.Hours.c
  YRs=sort(unique(a$FINYEAR))
  YR=1:length(YRs)
  fn.plot=function(dat,MAIN)
  {
    a1=aggregate(Km.Gillnet.Hours.c~FINYEAR,dat,sum)
...
fn.vessl.cpue.yr=function(dat)  #check variability in vessels cpue and month cpue by year
{
  Yr=sort(unique(dat$FINYEAR)) 
  dat$cpue=dat$Catch.Target/dat$Km.Gillnet.Hours.c
...
fn.bin.pred=function(dat,Int)   #Aggregate predictions by bins
{
...
fit.model.to.inspect=function(DATA,VES.No.K,Formula.cpue.pos,Vars,MIN.Wght,Int) #function for fitting model for checking predictions
{
    ...
    #log effort
    DataFile$log.Effort=log(DataFile$Km.Gillnet.Hours.c)
    
    
    # log catch other
    DataFile$log.Catch.Gummy=with(DataFile,ifelse(Catch.Gummy>0,log(Catch.Gummy),0))
    DataFile$log.Catch.Whiskery=with(DataFile,ifelse(Catch.Whiskery>0,log(Catch.Whiskery),0))
    DataFile$log.Catch.Scalefish=with(DataFile,ifelse(Catch.Scalefish>0,log(Catch.Scalefish),0))
    DataFile$log.Catch.Dusky=with(DataFile,ifelse(Catch.Dusky>0,log(Catch.Dusky),0))
    DataFile$log.Catch.Sandbar=with(DataFile,ifelse(Catch.Sandbar>0,log(Catch.Sandbar),0))
...
Effort.data.fun=function(DATA,target,ktch)
{
  ...
  ID=which(DATA$Km.Gillnet.Hours.c==0) #no effort records
  ID=c(ID,which(is.na(DATA$Km.Gillnet.Hours.c))) #NA records
  ...
  Effort.data=aggregate(cbind(Km.Gillnet.Hours.c,SOI,Freo)~zone+
                          FINYEAR+Same.return+MONTH+BLOCKX,Effort.data1,max)
  Effort.data=aggregate(cbind(Km.Gillnet.Hours.c,SOI,Freo)~zone+
                          FINYEAR+Same.return+MONTH+BLOCKX,Effort.data,sum)
  
  Effort.data.inv=aggregate(Km.Gillnet.Hours.inv~Same.return,Effort.data1,max)
  Effort.data.inv=aggregate(Km.Gillnet.Hours.inv~Same.return,Effort.data.inv,sum)
  ...
  #add effort without increase in efficiency
  dat$Km.Gillnet.Hours.c.no.creep=with(dat,ifelse(YEAR.c<1995,
                                                  Km.Gillnet.Hours.c/(1+Fish.Pow*(1+YEAR.c-1975)),Km.Gillnet.Hours.c))
...
Effort.data.fun.daily=function(DATA,target,ktch)
{
  ...
  Effort.data1=DATA[,Match.these.eff]
  Effort.data=aggregate(cbind(Km.Gillnet.Hours.c,SOI,Freo)~zone+
                          FINYEAR+Same.return.SNo+MONTH+BLOCKX,Effort.data1,max)
  Effort.data=aggregate(cbind(Km.Gillnet.Hours.c,SOI,Freo)~zone+
                          FINYEAR+Same.return.SNo+MONTH+BLOCKX,Effort.data,sum)
  
  Effort.data.inv=aggregate(Km.Gillnet.Hours.inv~Same.return.SNo,Effort.data1,max)
  Effort.data.inv=aggregate(Km.Gillnet.Hours.inv~Same.return.SNo,Effort.data.inv,sum)
  ...
  #reshape catch data
  TABLE=aggregate(cbind(Catch.Target,Catch.Gummy,Catch.Whiskery,Catch.Scalefish,Catch.Dusky,Catch.Sandbar)~MONTH+
                    FINYEAR+BLOCKX+VESSEL+Same.return.SNo+LAT+LONG+YEAR.c,data=DATA,sum,na.rm=T)
  TABLE=TABLE[order(TABLE$FINYEAR,TABLE$MONTH,TABLE$BLOCKX),]
  ...
  #merge catch and effort
  dat=merge(TABLE,Effort.data,by=c("Same.return.SNo","FINYEAR","MONTH","BLOCKX"),all.x=T)
  ...
  #add effort without increase in efficiency
  dat$Km.Gillnet.Hours.c.no.creep=dat$Km.Gillnet.Hours.c
...
{
  this=aggregate((Catch.Target/Km.Gillnet.Hours.c)~FINYEAR+MONTH,DATA,mean)
  Reshaped=(reshape(this,v.names = "(Catch.Target/Km.Gillnet.Hours.c)", idvar = "FINYEAR",
...
fn.plot.vess.selection=function(DAT,DAT1) #function for plotting number of vessels & catch for each data set
{
  ...
  if(i==2) mtext("Total catch of species (tons)                 ",2,cex=1.5,line=2.5,las=3)
}

check.cpue=function(DATA,NAME,BYDAY)   #function for checking cpue outliers
{
  DATA$Catch.Target=DATA$Catch.Target
  DATA$Km.Gillnet.Hours.c=DATA$Km.Gillnet.Hours.c
  
  if(BYDAY=="YES")
  {
    DATA$Catch.Target=DATA$Catch.Target/30
    DATA$Km.Gillnet.Hours.c=DATA$Km.Gillnet.Hours.c/30
  }
  
  DATA$CPUE=DATA$Catch.Target/DATA$Km.Gillnet.Hours.c
...
{
  this=aggregate((Catch.Target/Km.Gillnet.Hours.c)~FINYEAR+BLOCKX,DATA,mean)
  Reshaped=(reshape(this,v.names = "(Catch.Target/Km.Gillnet.Hours.c)", idvar = "FINYEAR",
...
fn.explore=function(DATA,NAMES)    #function for exploring data
{
  ...
  DATA$CPUE=with(DATA,Catch.Target/Km.Gillnet.Hours.c)
  ...
  TABLE.eff=aggregate(Km.Gillnet.Hours.c~FINYEAR,data=DATA,sum)
  #  TABLE.eff=aggregate(Km.Gillnet.Hours.c~FINYEAR,data=DATA,mean)
  TABLE.eff1=aggregate(Km.Gillnet.Hours.c~FINYEAR,data=DATA,sd)
  ...
  plot(1:nrow(TABLE8),TABLE8[,2]/1000,ylab="Total catch (tons)",xlab="Year",cex.lab=1.25,type="o",xaxt='n',pch=19)
...
fn.catch.cpue=function(DATA)   #function for plotting catch VS cpue
{
  ...
  mtext("Total catch (tons)",2,-1.15,outer=T,las=3,cex=1.35)
  ...
  mtext("Increase in catch and decrease in CPUE = increase in Q/efficiency",3)
}
...
fn.new.ves=function(DATA,Ves.No.ktch,Per.int)   #function for assigning vessels to categories
{
  ...
  PosData$cpue=PosData$Catch.Target/PosData$Km.Gillnet.Hours.c
  PosData$log.Catch=log(PosData$Catch.Target+0.00001)
  PosData$log.Effort=log(PosData$Km.Gillnet.Hours.c+0.00001)
...
#e.g. Inc.Dev.Exp(Dev.explained[[7]][,5],Dev.explained[[7]][,4])
...
fn.select.mod=function(DAT,dat.type,do.gamma)  #function for running model selection
{
  ...
  for (j in 1:N.species)
  {
    ...
    #log effort
    DataFile$log.Effort=log(DataFile$Km.Gillnet.Hours.c+0.00001)
...
Test.models=function(DATA,No.ves,SPEC,dat.type,MIN.Wght)   #function for testing models
{
  ...
  return(list(NAME.bi=NAME.bi,NAME.pos=NAME.pos))
}

Test.models.sandbar=function(DATA,No.ves,SPEC,dat.type,MIN.Wght)   #function for testing sandbar models
{
  #1. Create storing objects
  form.bin=vector("list",length=length(bin.form))
  form.pos.log=vector("list",length=length(pos.log.form))
...
load.and.select=function(dir)   
{
  setwd(dir)
  for(j in 1:N.species) All.models[[j]]=Names.models(SPECIES.vec[j])
  ...
  for(j in 1:N.species)
  {
    ...
    Dev.explained=list(bi=Dev.bin,pos=Dev.pos)
    BESTS[[j]]=list(AIC.bin=AIC.bin,AIC.Pos=AIC.Pos,Dev.explained=Dev.explained,
...
fn.stand.cpue=function(DATA,VES.No.K,SPEC,Formula.cpue,Formula.cpue.pos,Vars,Eff.Creep,MIN.Wght)
{
  #1. drop vessels that never reported catch of target species   
  DATA=subset(DATA,!(VESSEL%in%VES.No.K))
  #DATA=subset(DATA,!is.na(new.level.vessel))
  DATA=subset(DATA,Catch.Target>=MIN.Wght| Catch.Target==0)  #remove records with nonsense catch value
  ...
  #log effort
  if(Eff.Creep=="YES") DataFile$log.Effort=log(DataFile$Km.Gillnet.Hours.c)
  if(Eff.Creep=="NO") DataFile$log.Effort=log(DataFile$Km.Gillnet.Hours.c.no.creep)
  ...
  PosData$log.Catch=log(PosData$Catch.Target)  
  if(Eff.Creep=="YES") PosData$cpue=PosData$Catch.Target/PosData$Km.Gillnet.Hours.c
  if(Eff.Creep=="NO") PosData$cpue=PosData$Catch.Target/PosData$Km.Gillnet.Hours.c.no.creep  
  PosData$log.cpue=log(PosData$cpue)
...
see.BC.fit.to.data=function(DATA,VES.No.K,SPEC,Vars,Log.GLMbi,GLMlog,show.not.bias.corr,RespVar,MIN.Wght)
{
  #1. drop vessels that never reported catch of target species   
  DATA=subset(DATA,!(VESSEL%in%VES.No.K))
  #DATA=subset(DATA,!is.na(new.level.vessel))
  ...
  par(mfcol=c(2,1),mai=c(.6,.75,.3,.1),oma=c(1,1,1,1),las=1,mgp=c(2,.5,0))
  ...
  PosData$Pred=exp(predict(GLMlog,type="response"))
  ...
  if(RespVar=="catch")
  {
    a=aggregate(Catch.Target/1000~FINYEAR,PosData,sum)
    b=aggregate(Pred/1000~FINYEAR,PosData,sum)
  ...
  if(RespVar=="cpue")
  {
    ...
    LABS="CPUE (kg/km.gn.hr.)"
  }
  RANG=c(0,max(c(a[,2],b[,2])))   
...
fn.get.coef=function(dat,dat2)    #function for defining vessel and month levels from weighted average
{
  #get number of records
  TAB=function(dat2,what) Tabla=table(dat2[,match(what,names(dat2))])
  N.rec.ves=TAB(dat2,"VESSEL")
  N.rec.mnth=TAB(dat2,"MONTH")
...
Plot.Comb.cpue=function(BC,Fan,No.creep,Ind.ves.sens,No.wght,DAT,SPEC,REL,WHERE,Ves.no.ktch,Min.w)  #function for plotting sensitivity analysis
{
  DAT=subset(DAT,!VESSEL%in%Ves.no.ktch)
  a=get.foly.and.nominal(DAT,Ves.no.ktch,Min.w)
  Nominal=a$Nominal
  names(Nominal)[2]="Nominal"
  Foly=a$Foly
  Unstandardised=cbind(Nominal,Foly)
...
get.foly.and.nominal=function(DATA,VES.No.K,MIN.Wght)    #calculate foly and nominal cpue
{
  DATA=subset(DATA,!(VESSEL%in%VES.No.K))
  DATA=subset(DATA,Catch.Target>=MIN.Wght| Catch.Target==0)  
  DATA$cpue=DATA$Catch.Target/DATA$Km.Gillnet.Hours.c
  Nominal=aggregate(cpue~FINYEAR,DATA,mean)
  Agg.ktc=aggregate(Catch.Target~FINYEAR,DATA,sum,na.rm=T)
  Agg.eff=aggregate(Km.Gillnet.Hours.c~FINYEAR,DATA,sum,na.rm=T)
  Foly=Agg.ktc$Catch.Target/Agg.eff$Km.Gillnet.Hours.c
  return(list(Nominal=Nominal,Foly=Foly))
}
...
fn.prop.imputed=function(DAT,DAT1,SPEC,MOD)   #function for plotting impluted blocks
{
  ...
  Breaks=quantile(unlist(as.matrix(dat[,2:ncol(dat)])),probs=seq(0,1,1/numInt),na.rm=T)
  colLeg=(rep(c("black",rep("transparent",numberLab-1)),(numInt+1)/numberLab))
...
Influence.fn=function(MOD,DAT,Term.type,termS,LABEL,XLIM,SCALE)  #Bentley et al 2012
{
  ...
  for(p in 1:nt)MatcH[[p]]=match(Store2[[p]],names(Cofs))
  for(p in 1:nt)
  {
  ...
  for(p in 1:nt) if (Term.type[p]=="CAT") COEF.list[[p]]=data.frame(Dummy=Store1[[p]],coef=COEF.list[[p]])
  for(p in 1:nt) if (Term.type[p]=="CAT") COEF.list[[p]]$Dummy=as.character(COEF.list[[p]]$Dummy)
  ...
  for(p in 1:nt) if (Term.type[p]=="CAT") DAT=merge(DAT,COEF.list[[p]],by=termS[p],all.x=T)
  for(p in 1:nt)
  {
    if (Term.type[p]=="Cont")
    {
      DAT=cbind(DAT,rep(COEF.list[[p]],nrow(DAT)))
      colnames(DAT)[ncol(DAT)]=paste(termS[p],".coef",sep="")
    }
  ...
  for(p in 1:nt)
  {
    ...
    if (Term.type[p]=="CAT") Mean.coef[[p]]=mean(DAT[,Coef.vec[p]])
    #- Continuous variables
    if (Term.type[p]=="Cont")Mean.coef[[p]]=mean(DAT[,paste(termS[p],".coef",sep="")]*DAT[,termS[p]])    
  }
  ...
  for(p in 1:nt)
  {
    ...
    for(t in 1:length(ny))
    {
      ...
      if (Term.type[p]=="CAT") dev[t]=(sum(a[,Coef.vec[p]]-Mean.coef[[p]]))/ny[t]
      if (Term.type[p]=="Cont") dev[t]=(sum(a[,paste(termS[p],".coef",sep="")]*a[,termS[p]]-Mean.coef[[p]]))/ny[t]
    }
...
fn.bootstrap.index=function(DAT,Spc,vrs,best,Grid.d,Grid.d.zone,Ves.no,area.w,zonas,bloques,Min.w) #run whole boot procedure
{
    ...
    tryCatch(
{
  ...
  if(Boot.method=="DATA") Fit.BaseCase=fn.stand.cpue(Boot.Dat,Ves.no,Spc,best$Bi,best$Log,vrs,Eff.Creep="YES",Min.w)      
...
fn.folly=function(DAT,Ves.no.ktch,Min.w) #function for exporting unstandardised indices
{
  DAT=subset(DAT,!VESSEL%in%Ves.no.ktch)
  a=get.foly.and.nominal(DAT,Ves.no.ktch,Min.w)
  Nominal=a$Nominal
...
#1. Add SOI and Freo sea level data
    #monthly
...
#Remove initial years for sandbar as there are no records
Data.monthly.GN.sandbar=subset(Data.monthly.GN.sandbar,
      !(FINYEAR%in%c("1975-76","1976-77","1977-78","1978-79","1979-80","1980-81","1981-82",
      "1982-83","1983-84","1984-85")))

#Remove some remaining blocks outside species range
Data.monthly.GN.dusky=subset(Data.monthly.GN.dusky,!(BLOCKX%in%c("33200","34200")))
Data.monthly.GN.sandbar=subset(Data.monthly.GN.sandbar,!(BLOCKX%in%c("34180","35180","35181")))

Data.daily.GN.dusky=subset(Data.daily.GN.dusky,!(BLOCKX%in%c("33200","34200")))


#Remove daily data from monthly records
if(Separate.monthly.daily=="YES")
{
  Daily.years=unique(Data.daily.GN.dusky$FINYEAR)
  Data.monthly.GN.whiskery=subset(Data.monthly.GN.whiskery,!FINYEAR%in%Daily.years)
  Data.monthly.GN.gummy=subset(Data.monthly.GN.gummy,!FINYEAR%in%Daily.years)  
  Data.monthly.GN.dusky=subset(Data.monthly.GN.dusky,!FINYEAR%in%Daily.years)
  Data.monthly.GN.sandbar=subset(Data.monthly.GN.sandbar,!FINYEAR%in%Daily.years)  
}
...
#3. Remove spatial closures 
if(Remove.closure=="YES")
{
  #3.1. Remove Metro closure
...
# 
# Too.much.whis=with(Data.monthly.GN.whiskery,which(LIVEWT.c/BDAYS.c >3000))
# Too.much.gum=with(Data.monthly.GN.gummy,which(LIVEWT.c/BDAYS.c >3000))
# Too.much.dusk=with(Data.monthly.GN.dusky,which(LIVEWT.c/BDAYS.c >3000))
# Too.much.sand=with(Data.monthly.GN.sandbar,which(LIVEWT.c/BDAYS.c >3000))
# 
# Per.removed=function(DATA1,DATA2) per=round(100*DATA1/DATA2,2)
# 
# Per.removed.Too.much.catch=c(Per.removed(length(Too.much.whis),nrow(Data.monthly.GN.whiskery)),
#                              Per.removed(length(Too.much.gum),nrow(Data.monthly.GN.gummy)),
#                              Per.removed(length(Too.much.dusk),nrow(Data.monthly.GN.dusky)),
#                              Per.removed(length(Too.much.sand),nrow(Data.monthly.GN.sandbar)))
# names(Per.removed.Too.much.catch)=TARGETS.name                             
# 
# Removed.whis=Data.monthly.GN.whiskery[Too.much.whis,]
# Removed.gum=Data.monthly.GN.gummy[Too.much.gum,]
# Removed.dusk=Data.monthly.GN.dusky[Too.much.dusk,]
# Removed.sand=Data.monthly.GN.sandbar[Too.much.sand,]
# 
# Data.monthly.GN.whiskery=Data.monthly.GN.whiskery[-Too.much.whis,]
# Data.monthly.GN.gummy=Data.monthly.GN.gummy[-Too.much.gum,]
# Data.monthly.GN.dusky=Data.monthly.GN.dusky[-Too.much.dusk,]
# Data.monthly.GN.sandbar=Data.monthly.GN.sandbar[-Too.much.sand,]

# one=nrow(Data.monthly.GN.whiskery);two=nrow(Data.monthly.GN.gummy)
# three=nrow(Data.monthly.GN.dusky);four=nrow(Data.monthly.GN.sandbar)


  #4.2 Explore proportion of dusky and copper shark
    #monthly
...
axis(1,1:nrow(Prop.copper_dusky),Prop.copper_dusky$FINYEAR)
  ...
  #4.2 Remove NA effort
    #monthly
Data.monthly.GN.whiskery=subset(Data.monthly.GN.whiskery,!is.na(Km.Gillnet.Hours.c))
Data.monthly.GN.gummy=subset(Data.monthly.GN.gummy,!is.na(Km.Gillnet.Hours.c))
Data.monthly.GN.dusky=subset(Data.monthly.GN.dusky,!is.na(Km.Gillnet.Hours.c))
Data.monthly.GN.sandbar=subset(Data.monthly.GN.sandbar,!is.na(Km.Gillnet.Hours.c))

    #daily
Data.daily.GN.whiskery=subset(Data.daily.GN.whiskery,!is.na(Km.Gillnet.Hours.c))
Data.daily.GN.gummy=subset(Data.daily.GN.gummy,!is.na(Km.Gillnet.Hours.c))
Data.daily.GN.dusky=subset(Data.daily.GN.dusky,!is.na(Km.Gillnet.Hours.c))
Data.daily.GN.sandbar=subset(Data.daily.GN.sandbar,!is.na(Km.Gillnet.Hours.c))


#5. Put data into a list  
...
Species.list.daily=list(whiskery=Data.daily.GN.whiskery,gummy=Data.daily.GN.gummy,
  ...
  #5.1 Show folly effect: mean Vs sum
Show.folly.eg="NO"
if(Show.folly.eg=="YES")
{
  ...
  # mtext("CPUE (kg/km.gn.h)",2,outer=T,las=3,cex=1.5,line=-1.5)
...
#6.1 Number of vessels per species range
N.VES=vector('list',N.species)
names(N.VES)=names(Species.list)
for (i in 1:N.species)N.VES[[i]]=length(unique(c(Species.list[[i]]$VESSEL,Species.list.daily[[i]]$VESSEL)))

#.6.2 number of records in glms
if(Separate.monthly.daily=="YES")
{
  N.recs=N.VES
  N.recs.daily=N.VES
  for (i in 1:N.species)N.recs[[i]]=fun.check.numb.rec(Species.list[[i]])
  for (i in 1:N.species)N.recs.daily[[i]]=fun.check.numb.rec(Species.list.daily[[i]])
}



#7. Construct wide database for analysis 
#note: this constructs a single record for each year-month-vessel-block-gear with catch of target 
#       and other species as separate columns, giving a 0 catch for column "target" if no catch

  #7.1 variables used in standardisation
    #monthly
THESE.VARS=c("Same.return","YEAR.c","MONTH","VESSEL","BLOCKX","LAT","LONG")
Match.this=match(THESE.VARS,names(Data.monthly.GN.whiskery))
These.efforts=c("FINYEAR","Same.return","Km.Gillnet.Hours.inv",
                "Km.Gillnet.Hours.c","SOI","Freo","zone","MONTH","BLOCKX")

    #daily
THESE.VARS.daily=c("Same.return.SNo","YEAR.c","MONTH","VESSEL","BLOCKX","LAT","LONG")
Match.this.daily=match(THESE.VARS.daily,names(Data.daily.GN.whiskery))
These.efforts.daily=c("FINYEAR","Same.return.SNo","Km.Gillnet.Hours.inv",
                "Km.Gillnet.Hours.c","SOI","Freo","zone","MONTH","BLOCKX")


  #7.2 extract monthly records by Year-Month-Vessel-Block
#note: only select "Good" records (the variable "Reporter" includes bad catch and bad effort)
    #monthly
...
#Remove blocks on boundary of zones
Boundary.Blks=c(3416,3516,3616)
for (i in 1:N.species) DATA.list.LIVEWT.c[[i]]=subset(DATA.list.LIVEWT.c[[i]],!BLOCKX%in%Boundary.Blks)


#subset sandbar data  (San.areas.range)
DATA.list.LIVEWT.c[[4]]=subset(DATA.list.LIVEWT.c[[4]],LAT>=(-30) & LAT<(-26))
DATA.list.LIVEWT.c.sens.ind.ves[[4]]=subset(DATA.list.LIVEWT.c.sens.ind.ves[[4]],LAT>=(-30) & LAT<(-26))

#Keep records from indicative vessels only
Indi.ves=Indi.ves.sens=DATA.list.LIVEWT.c
DATA.list.LIVEWT.c.sens.ind.ves=DATA.list.LIVEWT.c
Indi.ves.daily=DATA.list.LIVEWT.c.daily
...
DATA.list.LIVEWT.c.daily.sens=DATA.list.LIVEWT.c.daily


#Criteria.indi='all'  #criteria for selecting sensitivity of indicative vessel
Criteria.indi='subset'  #use differenct criteria for indicative vessels

for(i in 1:N.species)
{
  #monthly
  second.criteria="top.percent"
  Indi.ves[[i]]=fn.indicative(Store.DATA.list.LIVEWT.c[[i]],Threshold.n.yrs,Min.rec.ves,second.criteria,90)
  Indi.ves.sens[[i]]=fn.indicative(Store.DATA.list.LIVEWT.c[[i]],Threshold.n.yrs.sens,Min.rec.ves.sens,second.criteria,90)
  ...
  if(second.criteria=="median.fishery")
  {
    DATA.list.LIVEWT.c[[i]]=subset(DATA.list.LIVEWT.c[[i]],VESSEL%in%Indi.ves[[i]]$Indic.ves)
    DATA.list.LIVEWT.c.sens.ind.ves[[i]]=subset(DATA.list.LIVEWT.c[[i]],VESSEL%in%Indi.ves.sens[[i]]$Indic.ves)
  }
  
  if(Separate.monthly.daily=="YES")
  {
    #daily
    Indi.ves.daily[[i]]=fn.indicative(DATA.list.LIVEWT.c.daily[[i]],Threshold.n.yrs,Min.rec.ves)
    a=fn.indicative(DATA.list.LIVEWT.c.daily[[i]],Threshold.n.yrs,Min.rec.ves)  
    DATA.list.LIVEWT.c.daily[[i]]=subset(DATA.list.LIVEWT.c.daily[[i]],VESSEL%in%Indi.ves.daily[[i]]$Indic.ves)
    DATA.list.LIVEWT.c.daily.sens[[i]]=subset(DATA.list.LIVEWT.c.daily[[i]],VESSEL%in%a$Indic.ves)
  }
}



#Plot vessels and catch
setwd("C:/Matias/Analyses/Catch and effort/Outputs/Paper")
tiff(file="Figure 8. Vessels and Catch by year.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")    
par(mfrow=c(4,2),mai=c(.45,.5,.1,.1),oma=c(.4,.1,.1,.95),mgp=c(1.5,.6,0),las=1)
for(i in 1:N.species) 
{
  fn.plot.vess.selection(Store.DATA.list.LIVEWT.c[[i]],DATA.list.LIVEWT.c[[i]])
  mtext(SPECIES.vec[i],4,las=3,line=0.5,cex=1.25)
}
mtext("Financial year",1,outer=T,line=-.75,cex=2)
dev.off()
...
unlist(Prop.vessels.dropped)
unlist(Prop.catch.explained)
...
#7.4.Table of years by block for each species within its' effective area
  #monthly
...
#7.4.2 Vessels-Years with catch
...
for(i in 1:N.species)
{
  ...
  {
    plt.ktch.yr_mn(subset(DATA.list.LIVEWT.c[[i]],select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,MONTH)))
  }
  if(Separate.monthly.daily=="YES")
  {
    plt.ktch.yr_mn(
      rbind(subset(DATA.list.LIVEWT.c[[i]],select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,MONTH)),
            subset(DATA.list.LIVEWT.c.daily[[i]],select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,MONTH))))
  }
  dev.off()
}
...
#7.5 Check outliers in catch and effort for removing nonsense values (Are quantiles meaningful ??)
...
if(Separate.monthly.daily=="YES")
{
  check.cpue.whis.daily=check.cpue(DATA.list.LIVEWT.c.daily[[1]],names(DATA.list.LIVEWT.c.daily)[1],BYDAY="NO")
  check.cpue.gum.daily=check.cpue(DATA.list.LIVEWT.c.daily[[2]],names(DATA.list.LIVEWT.c.daily)[2],BYDAY="NO")
  check.cpue.dus.daily=check.cpue(DATA.list.LIVEWT.c.daily[[3]],names(DATA.list.LIVEWT.c.daily)[3],BYDAY="NO")
  check.cpue.san.daily=check.cpue(DATA.list.LIVEWT.c.daily[[4]],names(DATA.list.LIVEWT.c.daily)[4],BYDAY="NO")
}
...
#8.1 Display proportion with and without catch for each species
...
if(Separate.monthly.daily=="NO")
{
  ...
  tiff(file="Figure 7.Prop of records with 0 catch.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")    
  ...
  mtext("Catch per record (Tons)",side=1,line=1,font=1,las=0,cex=1.25,outer=T)
...
if(Separate.monthly.daily=="YES")
{
  ...
  tiff(file="Figure 7.Prop of records with 0 catch.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")    
  par(mfcol=c(2,2),mar=c(1.5,1,1,1),oma=c(2.5,3,.1,.1),las=1,mgp=c(.1,.7,0))
  for ( i in 1:N.species)
  {
    ZeroKTC=DATA.list.0.catch.2[[i]]$proportion
    ...
    polygon(x=c(1,match("2005-06",LABS),match("2005-06",LABS),1),
            y=c(-0.5,-0.5,1.1,1.1),col="grey75")
    polygon(x=c(match("2005-06",LABS),NN,NN,match("2005-06",LABS)),
            y=c(-0.5,-0.5,1.1,1.1),col="grey90")
    points(1:NN,ZeroKTC,las=2,type="b",pch=19,cex=1.25,lwd=1.25)
    box()
    axis(1,at=1:NN,labels=F,tck=-0.015)
    axis(1,at=seq(1,NN,by=5),labels=LABS[seq(1,NN,by=5)],tck=-0.03,cex.axis=1.25)
    axis(2,at=seq(0,1,.2),labels=seq(0,1,.2),tck=-0.03,cex.axis=1.25)
    legend("bottomright",SPECIES.vec[i],bty='n',cex=1.65)
    
  }
  mtext("Financial year",side=1,line=1,font=1,las=0,cex=1.75,outer=T)
  mtext("Proportion of records with catch",side=2,line=1.5,font=1,las=0,cex=1.75,outer=T)
  dev.off()
  
}


#8.1 Plot missing year-blocks for each species
...
for (i in 1:N.species)
{
  ...
  if(Separate.monthly.daily=="NO")
  {
    dummy=subset(dull,select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,BLOCKX))
  }
    
  if(Separate.monthly.daily=="YES")
  {
    dummy=rbind(subset(dull,select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,BLOCKX)),
          subset(DATA.list.LIVEWT.c.daily[[i]],select=c(Catch.Target,Km.Gillnet.Hours.c,FINYEAR,BLOCKX)))  
    }
  Missing.yr.blk[[i]]=plt.miss.yr_blk(dummy)
  dev.off()
}
c(unlist(Missing.yr.blk))



#9. Construct index of abundance

  #9.2. Standardised cpue

ZONES=c("West","Zone1","Zone2")
Predictors=c("FINYEAR","BLOCKX","MONTH","SOI","Freo","VESSEL","Catch.Gummy","Catch.Whiskery",
             "Catch.Dusky","Catch.Sandbar","Km.Gillnet.Hours.c") 
VARIABLES=c("Catch.Target",Predictors)

Zone.list=list(c("West","Zone1","Zone2"),c("Zone1","Zone2"),c("West","Zone1","Zone2"),c("West","Zone1","Zone2"))


    #9.2.1. Explore data sets
...
for ( i in 1:N.species)
{
  ...
  #Number of vessels and blocks per zone and species
    #monthly
...
if(Model.run=="First")
{
  for(i in 1:N.species)
  {
    ...
    PREDS=Predictors[-match("Km.Gillnet.Hours.c",Predictors)]
    ...
    boxplot(DATA$cpue~DATA$FINYEAR,main="Outliers in response var?",ylab="cpue (kg/km.gn.hr)")
    dev.off()
...
#3.2.3 Define best model and error structure 

Red.th=2  #percentage reduction in deviance to accept term
if(Combine.ves=="YES") VARIABLES=c(VARIABLES,"new.level.vessel")
Fit.to.what="cpue"

#3.2.3.1.2 With Yr-Blk interactions
if(Def.mod.Str.=="YES" & With.interact=="YES")
{
  ...
  for(s in 1:N.species)
  {
    ...
    id=match("VESSEL",PREDS)
    PREDS[(id+1):length(PREDS)]=paste("log.",PREDS[(id+1):length(PREDS)],sep="")
    ...
    #{
      ...
      for(x in 1:length(bin.form)) bin.form[[x]]=as.formula(paste(paste("Catch.Target", "~", collapse=NULL), bin.form[[x]])) 
    #}
    ...
    for(x in 1:length(pos.log.form)) pos.log.form[[x]]=as.formula(paste(paste("log.cpue", "~", collapse=NULL), pos.log.form[[x]])) 
    
    Species.model[[s]]=list(bin.form=bin.form,pos.log.form=pos.log.form)
  ...
  setwd(Mon.wd)
  system.time(for(j in 1:N.species)
  {
    bin.form=Species.model[[j]]$bin.form
    pos.log.form=Species.model[[j]]$pos.log.form
    Test.models(DATA.list.LIVEWT.c[[j]],Ves.no.ktc[[j]],SPECIES.vec[j],"monthly",Min.weight[j])
  ...
  #Best models
  #Monthly
  Monthly.best=load.and.select(Mon.wd)
  ...
  for(j in 4:4)
  {
    ...
    for(w in 1: length(San.areas))
    {
      ...
      Monthly.best[[w]]=Test.models.sandbar(dummy,Ves.no.ktc[[j]],paste(Q,SPECIES.vec[j],sep=""),"monthly",Min.weight[j])      
    }
  }
  ...
  {
    setwd(Dai.wd)
    system.time(for(j in 1:N.species)Test.models(DATA.list.LIVEWT.c.daily[[j]],Ves.no.ktc[[j]],SPECIES.vec[j],"daily",Min.weight[j]))
    Daily.best=load.and.select(Dai.wd)
  ...
  #3.2.3.2 K-fold cross validation approach
Do.K_n.fold.test="NO"
...
Best.Model$'Whiskery shark'=list(
  Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+VESSEL+MONTH+log.Catch.Gummy+log.Catch.Dusky), 
  Log=as.formula(log.cpue~FINYEAR*BLOCKX+VESSEL+MONTH))

Best.Model$'Gummy shark'=list(
  Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+VESSEL+MONTH+log.Catch.Whiskery+log.Catch.Dusky), 
  Log=as.formula(log.cpue~FINYEAR*BLOCKX+VESSEL+MONTH))

Best.Model$'Dusky shark'=list(
  Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+VESSEL+MONTH+log.Catch.Gummy+log.Catch.Whiskery),
  Log=as.formula(log.cpue~FINYEAR*BLOCKX+VESSEL+MONTH+log.Catch.Whiskery))

Best.Model$'Sandbar shark'=list(
  Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+VESSEL+MONTH+log.Catch.Gummy+log.Catch.Whiskery+log.Catch.Dusky),
  Log= as.formula(log.cpue ~FINYEAR+BLOCKX+VESSEL+MONTH+log.Catch.Whiskery))
...
if(Separate.monthly.daily=="YES")
{
  ...
  Best.Model.daily$'Whiskery shark'=list(
    Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+new.level.vessel+MONTH+log.Catch.other.shk+offset(log.Effort)), 
    Log=as.formula(log.Catch~FINYEAR*BLOCKX+new.level.vessel+MONTH+log.Catch.other.shk+offset(log.Effort)))
  Best.Model.daily$'Gummy shark'=list(
    Bi=NULL, 
    Log=as.formula(log.Catch~FINYEAR*BLOCKX+new.level.vessel+MONTH+offset(log.Effort)))
  Best.Model.daily$'Dusky shark'=list(
    Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+new.level.vessel+MONTH+offset(log.Effort)),
    Log=as.formula(log.Catch~FINYEAR*BLOCKX+new.level.vessel+MONTH+log.Catch.other.shk+offset(log.Effort)))
  Best.Model.daily$'Sandbar shark'=list(
    Bi=as.formula(Catch.Target~FINYEAR+BLOCKX+new.level.vessel+MONTH+offset(log.Effort)),
    Log=as.formula(log.Catch~FINYEAR*BLOCKX+new.level.vessel+MONTH+offset(log.Effort)))
...
#model vars
VARS=c(VARIABLES,"Km.Gillnet.Hours.c.no.creep")
improve="NO"
...
#3.2.5 Standardise catches

  #3.2.5.1 Apply best model to all scenarios
Stand.BaseCase=vector('list',length=N.species)
names(Stand.BaseCase)=SPECIES.vec
Stand.Fantasy=Stand.GoodRep=Stand.no.creep=Stand.ind.vess=Stand.BaseCase
Stand.BaseCase.daily=Stand.BaseCase

DATA.list.LIVEWT.c[[1]]=subset(DATA.list.LIVEWT.c[[1]],!BLOCKX==3426)#remove block 3426 as it has only 1 record for whiskery sharks
DATA.list.LIVEWT.c.sens.ind.ves[[1]]=subset(DATA.list.LIVEWT.c.sens.ind.ves[[1]],!BLOCKX==3426)


system.time(for ( i in 1:N.species)     #note: this takes ~ 5 mins
{
  if(do.sensitivity=="YES")
  {
    #No effort creep
    Stand.no.creep[[i]]=fn.stand.cpue(DATA.list.LIVEWT.c[[i]],Ves.no.ktc[[i]],SPECIES.vec[[i]],
                  Best.Model[[i]]$Bi,Best.Model[[i]]$Log,VARS,Eff.Creep="NO",Min.weight[i])
    
     #Indicative vessel sensitivity
     Stand.ind.vess[[i]]=fn.stand.cpue(DATA.list.LIVEWT.c.sens.ind.ves[[i]],Ves.no.ktc[[i]],SPECIES.vec[[i]],
                 Best.Model[[i]]$Bi,Best.Model[[i]]$Log,VARS,Eff.Creep="YES",Min.weight[i])
  }
...
if(Separate.monthly.daily=="YES")
{
  Stand.BaseCase.daily[[i]]=fn.stand.cpue(DATA.list.LIVEWT.c.daily[[i]],Ves.no.ktc[[i]],SPECIES.vec[[i]],
                                          Best.Model.daily[[i]]$Bi,Best.Model.daily[[i]]$Log,VARS,Eff.Creep="YES",Min.weight[i])
  ...
  #3.2.5.2 Plot model diagnostics and extract deviance explained
...
system.time(for ( i in 1:N.species)     
{
  ...
  see.BC.fit.to.data(DATA.list.LIVEWT.c[[i]],Ves.no.ktc[[i]],SPECIES.vec[[i]],
          VARS,Stand.BaseCase[[i]]$Bi,Stand.BaseCase[[i]]$LogN,"NO",RespVar=Fit.to.what,Min.weight[i])
  dev.off()
  ...
  if(Separate.monthly.daily=="YES")
  {
    ...
    see.BC.fit.to.data(DATA.list.LIVEWT.c.daily[[i]],Ves.no.ktc[[i]],SPECIES.vec[[i]],
                       VARS,Stand.BaseCase.daily[[i]]$Bi,Stand.BaseCase.daily[[i]]$LogN,"NO",RespVar=Fit.to.what,Min.weight[i])
    dev.off()
  }
...
#Positive catch
tiff("Figure 9.monthly.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")
...
if(Separate.monthly.daily=="YES")
{
  tiff("Figure 8.daily.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")
  par(mfcol=c(3,4),las=1,mar=c(3,4,1.75,0.1),oma=c(1,.1,.1,.1),las=1,mgp=c(2,.5,0),cex.axis=1.25,cex.lab=1.1)
...
#3.2.6 Predict standardised catch
Bi.same.Pos="YES"  #set binomial new data same as pos new data

  #3.2.6.1 create Base case data 
...
for ( i in 1:N.species) BLOCKX[[i]]=fn.extract(sort(unique(DATA.list.LIVEWT.c[[i]]$BLOCKX)))
...
if(Sel.lev=='weighted.aver')
{
  for ( i in 1:N.species)
  {
    a=fn.get.coef(Stand.BaseCase[[i]],DATA.list.LIVEWT.c[[i]])
...
for ( i in 1:N.species)
{
  ...
  log.Effort[i]=log(Mn.cov(DATA.list.LIVEWT.c[[i]],"Km.Gillnet.Hours.c"))
}
 

  #3.2.6.2 create Indicative vessel sensitivity
All.ves.dat.same.BC.dat="NO"   #use same new data for all vessels as for base case
...
if(Sel.lev=='weighted.aver')
{
  ...
  for ( i in 1:N.species)
  {
    if(!All.ves.dat.same.BC.dat=="YES") a=fn.get.coef(Stand.ind.vess[[i]],DATA.list.LIVEWT.c.sens.ind.ves[[i]])
    if(All.ves.dat.same.BC.dat=="YES") a=fn.get.coef(Stand.BaseCase[[i]],DATA.list.LIVEWT.c[[i]])
...
for ( i in 1:N.species)
{
  ...
  if(!All.ves.dat.same.BC.dat=="YES")
  {
    ...
    log.Effort.sens.ind.ves[i]=log(Mn.cov(DATA.list.LIVEWT.c.sens.ind.ves[[i]],"Km.Gillnet.Hours.c"))
    
  }
}


  #3.2.6.3 create no effort creep sensitivity
Creep.dat.same.BC.dat="YES"  #use same new data for preciting effort creep
...
if(Sel.lev=='weighted.aver')
{
  ...
  for ( i in 1:N.species)
  {
    if(!Creep.dat.same.BC.dat=="YES") a=fn.get.coef(Stand.no.creep[[i]],DATA.list.LIVEWT.c[[i]])
    if(Creep.dat.same.BC.dat=="YES") a=fn.get.coef(Stand.BaseCase[[i]],DATA.list.LIVEWT.c[[i]])
    if(!Bi.same.Pos=="YES") dummy=list(bi=a$ID.mnth.bi,pos=a$ID.mnth.pos)
    if(Bi.same.Pos=="YES") dummy=list(bi=a$ID.mnth.pos,pos=a$ID.mnth.pos)
    Unik.mn=fn.extract(sort(unique(DATA.list.LIVEWT.c[[i]]$MONTH)))
...
if(Separate.monthly.daily=="YES")
{
  ...
  for ( i in 1:N.species) BLOCKX.daily[[i]]=fn.extract(sort(unique(DATA.list.LIVEWT.c.daily[[i]]$BLOCKX)))
  MONTH.daily=MONTH
  
  new.MN.daily=new.MN
  for ( i in 1:N.species)new.MN.daily[[i]]=Most.common(DATA.list.LIVEWT.c.daily[[i]],"MONTH")
  for ( i in 1:N.species)new.MN.daily[[i]]=MONTH.daily[as.numeric(new.MN.daily[[i]])]
  
  
  new.level.vessel.daily=new.level.vessel
  for ( i in 1:N.species)
  {
    new.level.vessel.daily[[i]]=Most.common(DATA.list.LIVEWT.c.daily[[i]],"new.level.vessel")
    Unik.ves=sort(unique(DATA.list.LIVEWT.c.daily[[i]]$new.level.vessel))
    new.level.vessel.daily[[i]]=Unik.ves[match(new.level.vessel.daily[[i]],Unik.ves)]
  }
  
  FREO.daily=soi.daily=KTch.Other.daily=Eff.rt.daily=vector(length=N.species)
  for ( i in 1:N.species)
  {
    FREO.daily[i]=Mn.cov(DATA.list.LIVEWT.c.daily[[i]],"Freo")
    soi.daily[i]=Mn.cov(DATA.list.LIVEWT.c.daily[[i]],"SOI")
    KTch.Other.daily[i]=log(Mn.cov(DATA.list.LIVEWT.c.daily[[i]],"Catch.other.shk"))
    Eff.rt.daily[i]=Mn.cov(DATA.list.LIVEWT.c.daily[[i]],"Km.Gillnet.Hours.c")
  }
}
...
Grid.data.daily=Grid.data
  ...
  #Base case data set for predictions
Interaction.sandbar="NO"  #if not fitting a yr-block interaction for sandbars
...
for ( i in 1:N.species)
{
  ...
  if(i==4)
  {
    ...
    if(Interaction.sandbar=="NO")
    {
      BLOCKX[[i]]=subset(BLOCKX[[i]],BLOCKX[[i]]==Most.common(DATA.list.LIVEWT.c[[i]],"BLOCKX"))
    }
   }
...
if(Separate.monthly.daily=="YES")
{
  for ( i in 1:N.species)
  {
    dummy=expand.grid(FINYEAR=FINYEAR.daily,BLOCKX=BLOCKX.daily[[i]],MONTH=new.MN.daily[[i]],
          VESSEL=new.level.vessel.daily[[i]],log.Catch.other.shk=KTch.Other.daily[i],
          log.Effort=log.Effort.daily,Freo=FREO.daily[i],SOI=soi.daily[i])
    Grid.data.daily[[i]]=list(bin=dummy,pos=dummy) 
  }
}
...
if(do.sensitivity=="YES")
{
  ...
  for ( i in 1:N.species)
  {
    ...
    if(i==4)
    {
      ...
      if(Interaction.sandbar=="NO")
      {
        BLOCKX.sens.ind.ves[[i]]=subset(BLOCKX.sens.ind.ves[[i]],
            BLOCKX.sens.ind.ves[[i]]==Most.common(DATA.list.LIVEWT.c.sens.ind.ves[[i]],"BLOCKX"))
      }
  ...
  for ( i in 1:N.species)
  {
    log.Effort.no.creep[i]=log(Mn.cov(DATA.list.LIVEWT.c[[i]],"Km.Gillnet.Hours.c.no.creep"))
...
#3.2.7 Imputation of missing Year-block 
#rules: (combination of Punt et al 2000 and Carruthers et al 2011)
        #1.   If NAs prior to fishing, then fill in with average first 3 records with data
        #2.   If NAs after fishing, then fill in with average from last 3 years of data
        #3.   If NAs in between records, then use linear interpolation
impute.aver=3


#3.2.8 Construct index 

  #3.2.8.1 Block weights     
...
AREA.W$"Whiskery shark"=rbind(AREA.W$"Whiskery shark",data.frame(BLOCKX=3521,Fish.Area=0.01,MaxDepth=200))
AREA.W$"Gummy shark"=Gum.fishArea  
AREA.W$"Dusky shark"=Dusky.fishArea
AREA.W$"Dusky shark"=rbind(AREA.W$"Dusky shark",data.frame(BLOCKX=3319,Fish.Area=0.01,MaxDepth=200))
AREA.W$"Sandbar shark"=Sand.fishArea
  

#3.2.9. Compare indices in sensitivity test
Pred.st.BaseCase=vector('list',length=N.species)
names(Pred.st.BaseCase)=SPECIES.vec
Pred.st.Fantasy=Pred.st.no.creep=Pred.st.ind.ves.sens=Pred.st.no.area.weight=Pred.st.BaseCase
Pred.st.BaseCase.daily=Pred.st.BaseCase
Yr_Blks.imputed=Prop.yr_blks.imputed=Prop.yr_blks.imputed.daily=Pred.st.BaseCase

  #3.2.9.1 Construct indices for sensitivity scenarios
system.time(for (i in 1:N.species)
{
    #Base case
  ...
  if(do.sensitivity=="YES") 
  {
    ...
    Pred.Fantasy=Pred.no.weight=Stand.BaseCase[[i]]
   
      #predict catch
    Pred.ind.ves=predict.ktch(Pred.ind.ves,Grid.data.sens.ind.ves[[i]],SPECIES.vec[[i]])
    Pred.Fantasy=predict.ktch(Pred.Fantasy,Grid.data[[i]],SPECIES.vec[[i]])
    ...
    rm(Pred.BaseCase,Pred.ind.ves,Pred.no.weight,Pred.no.creep,Pred.Fantasy)
  }

    #daily
  if(Separate.monthly.daily=="YES")
  {
    Pred.BaseCase=Stand.BaseCase.daily[[i]]  
    
    #predict catch
    Pred.BaseCase=predict.ktch(Pred.BaseCase,Grid.data.daily[[i]],SPECIES.vec[[i]])         
    ...
    #construct index   
    Pred.st.BaseCase.daily[[i]]=construct.index(Pred.BaseCase,AREA.W[[i]],"YES",SPECIES.vec[[i]],
                Weight.by.area="YES")  
  }
})

  #3.9.2 Compare indices
#define if expressing in relative terms or not
RELATIVE="YES"
#RELATIVE="NO"

  #define relative to what (note that relative to 1st year reduces uncertainty)
relative.to="Mean.1"
...
if(do.sensitivity=="YES")
{
  ...
  for ( i in 1:N.species)
  {
      ...
      Plot.Comb.cpue(Pred.st.BaseCase[[i]],Pred.st.Fantasy[[i]],Pred.st.no.creep[[i]],
                     Pred.st.ind.ves.sens[[i]],Pred.st.no.area.weight[[i]],DATA.list.LIVEWT.c[[i]],
      ...
      if(RELATIVE=="NO") mtext("Catch rate (kg/km.gn.h)",side=2,line=2.2,font=1,las=0,cex=2)
    dev.off()
  }
  ...
  tiff(file="Figure 10.Cpue.comparison.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")    
    ...
    for ( i in 1:N.species)
    {
      Plot.Comb.cpue(Pred.st.BaseCase[[i]],Pred.st.Fantasy[[i]],Pred.st.no.creep[[i]],
                     Pred.st.ind.ves.sens[[i]],Pred.st.no.area.weight[[i]],DATA.list.LIVEWT.c[[i]],
                     SPECIES.vec[i],REL=RELATIVE,where[i],Ves.no.ktc[[i]],Min.weight[i])
      if(i==1)legend('topright',c(SPECIES.vec[i],"  ",c("Nominal","Folly","Fantasy","No effort creep","Ind. vessels sens.",
                                                        "No area weighting","Base case")),col=c(rep("transparent",2),COLORES),
                     lty=c(1,1,LINEAS),bty='n',cex=1.4,lwd=c(1,1,LWD))
      if(!i==1)legend("topright",SPECIES.vec[i],bty='n',cex=1.4)
    }
    ...
    if(RELATIVE=="NO") mtext("Catch rate (kg/km.gn.h)",side=2,line=-1.35,font=1,las=0,cex=1.75,outer=T)
  dev.off() 
  ...
  if(compare.creep.sep=="YES")
  {
    ...
    mtext("CPUE (kg/km.gn.h)",side=2,line=-2,font=1,las=0,cex=1.75,outer=T)
    mtext("Financial year",side=1,line=1,font=1,las=0,cex=1.75,outer=T)
    dev.off()
  }
}
...
#3.10 Plot sandbar spatio-temporal catches
...
if(Do.sandbar.explore=="YES")
  ...
  #a$cpue=a$Catch.Target/a$Km.Gillnet.Hours.c
  a$LAT=a$LAT-.5
  a$LONG=a$LONG+.5
...
#3.11 Explain the influence of the different model terms
...
if(do.influence=="YES")
{
  ...
  tiff(file="Figure12.Imputed.blks_by_yr.tiff",width = 2400, height = 2400,units = "px", res = 300, compression = "lzw")    
...
#   for(i in 1:N.species) fn.prop.imputed(Grid.data[[i]]$pos,Yr_Blks.imputed[[i]],SPECIES.vec[i],Stand.BaseCase[[i]]$LogN)
  ...
  XLIM.vec=list(c(.25,3),c(.2,1.4),c(.5,3.5),c(.5,1.75))
...
#ACA
#3.12.  Build confidence intervals and abundance index for stock assessments
#note: it takes 20 hours for 4 species, 1000 boots and annual and monthly indices
system.time(if(Do.boot=="YES")
{
  #note:  Boot.method="DATA" resamples original data in a stratified way to keep all year-blocks
  #       Boot.method="RESIDUALS uses Haddon 2001 (page 306) resampling of residuals approach
  
    #define bootstrapping method
  #Boot.method="DATA"
  ...
  common.vars=c("Catch.Target","Km.Gillnet.Hours.c","BLOCKX","VESSEL","MONTH","FINYEAR",
  ...
  ZONES=list(Whis=names(Zones.whi),Gum=names(Zones.gum),Dus=names(Zones.dus),San=names(Zones.san))
  ...
  if(Boot.method=="RESIDUALS")
  {
    ...
    for ( i in 1:N.species)
    {
      ...
      AA=fn.bootstrap.index(DATA.list.LIVEWT.c[[i]],SPECIES.vec[[i]],
            ...
            AREA.W[[i]],ZONES[[i]],Blks.in.zn[[i]],Min.weight[i])
  ...
  rm(Stand.no.creep,Data.Summary,DATA.list.LIVEWT.c.daily,
     ...
     #3.12.1.2 bootstrapping data
  if(Boot.method=="DATA")
  {
    #create objects for storing boots
    Pred.st.BaseCase.CI=vector('list',length=N.species)
    names(Pred.st.BaseCase.CI)=SPECIES.vec
    Pred.st.BaseCase.CI.by.zone=Pred.st.BaseCase.CI
...
#     Colr2="darkgreen"
  ...
  if(RELATIVE=="NO") mtext("CPUE (kg/km.gn.h)",side=2,line=1,font=1,las=0,cex=2.5,outer=T)
  dev.off()
  ...
  if(RELATIVE=="NO") mtext("CPUE (kg/km.gn.h)",side=2,line=1,font=1,las=0,cex=1.5,outer=T)
  dev.off()
  ...
  for ( i in 1:N.species)
  {
    dat=fn.folly(DATA.list.LIVEWT.c[[i]],Ves.no.ktc[[i]],Min.weight[i])
  ...
  {    
    ...
    dat=data.frame(Finyear=a$FINYEAR,Mean=a$MEAN,
          CV=a$CV,LOW.CI=a$LOW.CI,UP.CI=a$UP.CI)
  ...
  for ( i in 1:N.species)
  {
    ...
    for(z in 1:length(zns))
    {
      dat=data.frame(Finyear=a$FINYEAR,Mean=a$MEAN[[z]],
                     CV=a$CV[[z]],LOW.CI=a$LOW.CI[[z]],UP.CI=a$UP.CI[[z]])
  ...
  for ( i in 1:N.species)
  {
    ...
    plot(a[,2],type='l',ylim=c(0,max(a[,2:ncol(a)])),lwd=LWD,lty=LT[1],ylab='',xaxt='n',xlab='',cex.axis=1.25)
    if(length(zNs)>1)for(z in 2:length(zNs)) lines(a[,z+1],lwd=LWD,lty=LT[z])

    legend('topright',zNs,bty='n',cex=1.25,lty=LT,lwd=LWD)
...
# {
#   DATA=subset(DATA,!is.na(Km.Gillnet.Hours.inv))
...
#     Aggregated.effort=aggregate(Km.Gillnet.Hours.inv~FINYEAR,data=DATA,FUN=sum,na.rm=T)
...
#         Ag.Eff=aggregate(Km.Gillnet.Hours.c~FINYEAR,data=DATA1,FUN=sum,na.rm=T) 
...
#     simple.mixed=lme((Catch.Target/Km.Gillnet.Hours.c)~1, data = DATA, random = ~ 1 |VESSEL)
...
#     
#     a=aggregate((Catch.Target/Km.Gillnet.Hours.c)~VESSEL,DATA,mean)
#     b=aggregate((Catch.Target/Km.Gillnet.Hours.c)~VESSEL,DATA,sd)
#     a=merge(a,b,by="VESSEL")
#     names(a)[2:3]=c("Mean","SD")
#     a=merge(a,vessel.coef,by="VESSEL",all.x=T)
#     
...
# }  
# mtext("                                                                                                 Total catch per vessel (tons)",
...
#   if(i%in%c(2,4))axis(1,at=seq(0,800,200),labels=seq(0,800,200),cex.axis=.7,tck=-0.03,padj=-0.5)
#   if(i%in%c(1,2))axis(2,at=seq(0,1000,250),labels=seq(0,1000,250),cex.axis=.7,tck=-0.03)
#   if(i==2)mtext("                       Total catch per block (tons)",side=2,line=1.65,font=1,las=0,cex=1,outer=F)
# }  
# 
# mtext("                                             Number of records",
...
#                              best$Log,vrs,Eff.Creep="YES",Min.w)
#   
#   #2.3. run predictions
#   Pred.BaseCase=predict.ktch(Fit.BaseCase,Grid.d,Spc)
#   
