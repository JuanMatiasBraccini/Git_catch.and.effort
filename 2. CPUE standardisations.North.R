...
Data=Data%>%
    filter(METHOD%in%c('LL') & Reporter=="good" & SPECIES<50000 & !SPECIES%in%c(22999,31000))%>%
...
Data=Data%>%filter(SPECIES%in%as.numeric(Keep.sp))
...
#standardise
fn.delta=function(d,Formula.gam,Formula.bi.gam)   #used for other species
{
...
{
  this=Keep.sp[s]
  fn.delta(d=Data.w[,-match(Keep.sp[-match(this,Keep.sp)],colnames(Data.w))]%>%
             rename(cpue=!!this),
           Formula.gam=formula(ln.cpue~FINYEAR+MONTH+VESSEL+ s(LAT,LONG)),
...
for(s in nnn)
{
    ...
    if(!is.null(Pred[[s]]))
    {
      ...
      if(!is.null(Effective[[s]]))
      {
        Effective[[s]]=Effective[[s]]%>%mutate(CV=se/mean)
      }
    }
  ...
  #daily
    if(!is.null(Pred.daily[[s]]))
    {
      ...
      {
        Effective_daily[[s]]=Effective_daily[[s]]%>%mutate(CV=se/mean)
      }
    }
  }
